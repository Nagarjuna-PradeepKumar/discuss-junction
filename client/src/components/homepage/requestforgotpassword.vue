    ...
    <v-row>
      <v-col>
        <v-card elevation="24" class="pa-6" dark min-height="300px">
          ...
          <v-card-subtitle class="text-center orange--text">Forgot password</v-card-subtitle>
          ...
          <v-row justify="center">
            ...
            <v-form ref="forgotpasswordform">
              <v-text-field
                ...
                color="orange"
              />
              <v-btn block small class="primary mt-5" @click.prevent="resend">submit</v-btn>
            </v-form>
          </v-row>
          <!-- Show signup here -->
...
export default {
  ...
  methods: {
    resend: function() {
      ...
      this.successmessage = "";
      this.overlay = true;
      this.$axios.post("/forgotpassword", this.send).then(Response => {
